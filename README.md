...
- **Advanced Exploratory Data Analysis (EDA)**: Performed in-depth statistical profiling and visualization on real-world datasets (House Prices, Customer Churn, Penguins, Titanic, etc.) to uncover non-obvious patterns and trends.
...
- **Advanced Data Cleaning & Preprocessing**: Built production-ready cleaning pipelines with missing value imputation strategies, outlier treatment using IQR methods, and RegEx-based text parsing for feature extraction.
- **Machine Learning Pipeline Development**: Created end-to-end ML workflows with Scikit-Learn including proper train-test splitting, feature scaling, dimensionality reduction (PCA), and supervised learning models achieving 95%+ accuracy.
- **Data Leakage Prevention**: Implemented industry-standard practices by fitting transformers only on training data and applying learned parameters to test sets, ensuring mathematically sound model validation.
...
- **High-Performance Numerical Computing**: Leveraged vectorized operations with NumPy for high-efficiency data manipulation, filtering, and weighted score calculations (100x+ faster than Python lists).
- **Professional Visualization Design**: Crafted multi-panel statistical visualizations (Box plots, Heatmaps, Distplots, Decision Trees) with Seaborn and Matplotlib to clearly communicate complex relationships.
...
- **Structured Project Architecture**: Organized the codebase into a clean, hierarchical structure with clear separation between data, scripts, notebooks, and visualization outputs.
...
├── Week 8 Seminar/       # Week 8 Seminar: Scikit-Learn ML Pipelines
│   ├── data/             # Raw datasets (Titanic, House Price, Iris)
│   ├── notebooks/        # Detailed interactive theory and workshops
│   │   ├── introduction_week_8.ipynb
│   │   └── Workshop_Week_8.ipynb
│   ├── scripts/          # Standalone ML pipeline automation
│   │   └── introduction_week_8.py
│   ├── visualizations/   # ML model outputs and decision trees
│   └── README.md         # Detailed Week 8 technical guide
...
#### Data Summary:
- **Total records**: 18,802
- **Loaded for exercise**: 1,000 (first 1,000 records)
- **Columns used**: meta_score, user_review
- **Missing values**: 3 NaN (0.15%)
  - meta_score: 1 missing
  - user_review: 2 missing (from "tbd" values)
...
- **Index**: House ID (101-1560)
- **Features**: Neighborhood, Condition, Sale Price, Year Built, Quality, etc.
- **Target**: SalePrice (ranging from $34,900 to $755
---
...
## 🧹 Week 7 Seminar: Advanced Data Cleaning & Processing

The **Week 7 Seminar** focuses on advanced data cleaning, transformation, and preprocessing techniques using real-world datasets.

### 📁 Structure
- **`introduction_week_7.py`**: Python recreation of Titanic data processing lecture.
- **`case_study_1/`**: House Price data cleaning and feature engineering.
- **`case_study_2/`**: Customer Churn data preprocessing pipeline.
- **`case_study_3/`**: Titanic dataset cleaning with RegEx name parsing.
- **`data source/`**: Raw datasets (titanic_train.csv, customer_churn.csv, house_price.csv).
- **`visualizations/`**: Generated plots and analysis outputs.

### 🛠 Technical Implementation

#### Case Study 1: House Price Processing
**File**: `exercise_1_case_study_1_titanic.py`
- Systematic missing value analysis and imputation strategies
...
**File**: `exercise_1_case_study_2_customer_churn.py`
- Business-specific data cleaning (tenure, contract type)
- Binary encoding for yes/no variables
- Handling mixed data types (numeric strings)
- Feature scaling and normalization

#### Case Study 3: Titanic Dataset Processing
**File**: `exercise_1_case_study_3_titanic.py`
- **7-Step Robust Cleaning Pipeline**:
  1. Handling missing Age values (median imputation by Pclass)
  2. Cabin letter extraction and grouping
  3. Embarked port imputation (mode)
  4. Fare outlier treatment (log transformation)
  5. Family size feature engineering
  6. Categorical encoding (Sex, Embarked)
  7. Data type optimization

**File**: `exercise_2_case_study_3_titanic.py`
- **Regular Expression Name Parsing**:
  - Extracts `Surname`, `Title`, and `FirstName` from passenger names
  - Pattern: `r'^([^,]+),\s*([^.]+)\.\s*(.*)$'`
  - Enables social status analysis via titles (Mr., Mrs., Miss., etc.)
  - Supports family relationship identification

### 📊 Key Learning Outcomes
- **Data Auditing**: Identifying hidden issues (whitespace, wrong types, systemic patterns)
- **Missing Data Strategy**: When to drop vs impute vs engineer new features
- **Outlier Management**: IQR method, capping, and log transformation
- **Feature Engineering**: Creating new informative columns from existing data
- **RegEx Mastery**: Using regular expressions for complex text parsing
- **Pipeline Thinking**: Building reproducible, sequential cleaning workflows

---

## 🤖 Week 8 Seminar: Machine Learning with Scikit-Learn

The **Week 8 Seminar** focuses on building robust machine learning pipelines using Scikit-Learn.

### 📁 Structure
- **`data/`**: Raw datasets (Titanic, House Price, Iris) used throughout the exercises.
- **`notebooks/`**: 
  - `introduction_week_8.ipynb`: Guided walkthrough of ML fundamentals including detailed theory.
  - `Workshop_Week_8.ipynb`: In-depth House Price prediction workshop updated with data leakage and scaling fixes.
- **`scripts/`**: 
  - `introduction_week_8.py`: Full pipeline script automating Loading, Processing, and Modeling.
- **`visualizations/`**: Generated model outputs such as Decision Trees.

### 🛠 Technical Implementation Highlights
- **Advanced Imputation**: Preserving data structure using `KNNImputer` and `IterativeImputer`.
- **Intelligent Scaling**: Applying `StandardScaler`, `MinMaxScaler`, and `RobustScaler` to normalize features.
- **Categorical Processing**: Using `OneHotEncoder` and `OrdinalEncoder` for mathematical compatibility.
- **Dimensionality Reduction**: Leveraging **PCA** with MLE to automatically select optimal feature counts.
- **Supervised Models**: Implementing **Decision Tree Classifiers/Regressors** and **Multiple Linear Regression**.

### 🚀 Key Improvements Made
- **Zero Data Leakage**: Restructured pipelines to split data *before* fitting transformers.
- **Encoding Optimization**: Fixed "Dimension Explosion" by targeting only categorical columns.
- **PCA Integrity**: Mandated scaling before PCA to ensure mathematically sound feature weighting.
